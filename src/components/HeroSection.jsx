...
export default function HeroSection({ dict, lang }) {
  ...
  return (
    <header className="relative overflow-hidden bg-white pb-24 pt-48 lg:pb-32 lg:pt-64">
      ...
      <Stagger className="relative z-10 mx-auto max-w-7xl px-8 text-center" stagger={0.1}>
        <StaggerItem className="mb-10 inline-flex items-center rounded-full border border-slate-100 bg-slate-50 px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500">
          {hero.badge}
        </StaggerItem>

        <StaggerItem
          as="h1"
          className="mx-auto mb-10 max-w-5xl text-5xl font-extrabold leading-[1.1] tracking-tight text-slate-900 lg:text-[5.5rem]"
        >
          {hero.titleLine1} <br />
          <span className="text-gradient">{hero.titleHighlight}</span>
        </StaggerItem>

        <StaggerItem className="mx-auto mb-12 max-w-3xl">
        ...
        <StaggerItem className="mb-24 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Link
            href={`/${lang}/pricing`}
        ...
        <StaggerItem className="mx-auto grid max-w-5xl grid-cols-1 gap-12 border-t border-slate-50 py-12 md:grid-cols-3">
