...
const PATH = "/industries/beauty-wellness";
...
export async function generateMetadata({ params }) {
  ...
  return buildLocalizedMetadata({ lang, path: PATH, seo: dict.pages.beautyWellness.seo });
}

export default async function BeautyWellnessPage({ params }) {
  const { lang } = await params;
  ...
  const dict = getDictionary(lang);
  const page = dict.pages.beautyWellness;
          ...
          <div className="mx-auto max-w-4xl px-8">
            <div className="rounded-[2rem] border border-slate-200/80 bg-white p-10 text-center shadow-[0_10px_35px_rgba(15,23,42,0.06)]">
              <span className="mb-4 block text-[11px] font-bold uppercase tracking-widest text-primary">
                {page.caseStudyComingSoon.label}
              </span>
              <div className="text-gradient mb-3 text-4xl font-black tracking-tight">
                {lang === "es" ? "Próximamente" : "Coming soon"}
              </div>
              <h3 className="mb-4 text-xl font-bold text-slate-900">
                {page.caseStudyComingSoon.title}
              </h3>
              <p className="mx-auto max-w-xl leading-relaxed text-slate-500">
                {page.caseStudyComingSoon.description}
              </p>
            </div>
          </div>
        </section>
